    ...
    return (
        <div className="flex flex-col gap-3 p-4 border rounded-md shadow-md">
            ...
            <span className="px-3 py-1 bg-primary text-white inline-block max-w-max rounded-default text-sm">
                {item.location?.country || t("No country")}
            </span>
            <h3 className="text-lg font-bold">
                {item.location?.address || t("No address")}
            </h3>
            <ul className="flex items-center gap-3">
                <li className="flex items-center gap-1">
                    ...
                    <span>{item.details?.bedrooms || t("N/A")}</span>
                </li>
                <li className="flex items-center gap-1">
                    <svg
                        stroke="currentColor"
            ...
            <p>
                {t("Price per Night")}:
                {item.price?.pricePerNight?.toLocaleString() || t("N/A")}{" "}
                {item.price?.currency || ""}
            </p>
